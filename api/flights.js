...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      let query = supabase.from('flights_destinations').select('*').order('created_at', { ascending: false });
  ...
  if (req.method === 'POST') {
    try {
      const body = req.body || {};
      const { data, error } = await supabaseAdmin.from('flights_destinations').insert([body]).select();
      if (error) throw error;
  ...
  if (req.method === 'PUT') {
    ...
    try {
      ...
      const { data, error } = await supabaseAdmin.from('flights_destinations').update(body).eq('id', id).select();
      if (error) throw error;
  ...
  if (req.method === 'DELETE') {
    if (!id) return res.status(400).json({ message: 'ID is required' });
    try {
      const { error } = await supabaseAdmin.from('flights_destinations').delete().eq('id', id);
      if (error) throw error;
      return res.json({ message: 'Flight destination deleted' });
    } catch (err) {
      return res.status(500).json({ message: err.message });
    }
  }
