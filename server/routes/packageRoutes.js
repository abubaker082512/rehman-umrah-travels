...
router.get('/', async (req, res) => {
  ...
  if (needsSeeding) {
    const seedList = [
      ...
      },
      {
        ...
        not_includes: ['Return Flight', 'Meals', 'Travel insurance', 'Laundry and room service charges'],
        itinerary: [
          {day: 'Day 01', title: 'Welcome to Makkah', description: 'Arrive in Makkah, transfer to Dar El Eiman Grand Makkah. Complete Umrah.'},
    ...
    await req.supabase.from('packages').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    await req.supabase.from('packages').insert(seedList);
  ...
  // Self-cleaning deletion for local database (non-conforming packages)
  if (data && data.length > 0) {
    ...
    if (idsToDelete.length > 0) {
      req.supabase.from('packages').delete().in('id', idsToDelete)
        .then(({ error: delErr }) => {
          if (delErr) console.error('[Cleanup] Failed to delete local old packages:', delErr.message);
        })
